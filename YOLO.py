...
#获取检测到的标签和中心坐标
    def Get_Center(self,res):
        centers = []
        if self.task_type=="detect" and res:
            for det in res:
                x1, y1, x2, y2 = map(lambda x: int(round(x, 0)), det[:4])
                raw_cx = (x1 + x2) / 2
                raw_cy = (y1 + y2) / 2

                disp_cx = raw_cx * self.display_size[0] // self.rgb888p_size[0]
                disp_cy = raw_cy * self.display_size[1] // self.rgb888p_size[1]

                labels = self.labels[int(det[5])]

                centers.append({
                    "raw_center": (raw_cx, raw_cy),
                    "disp_center": (disp_cx, disp_cy),
                    "label": labels
                })
        return centers
